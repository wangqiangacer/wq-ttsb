...
public class RedisBaseUtils {
    ...
    public <K, V> V get(K key, Class<V> javaType) {
        ...
        return JsonUtil.readJson(value, javaType);
    }

    public <K, V> V getT(K key, Class<V> javaType) {
        String value = stringRedisTemplate.opsForValue().get(key);
        if (StringUtils.isBlank(value)) {
            return null;
        }
        return JSON.parseObject(value, javaType);
    }

    public <K, V> V getV(K key) {
        RedisTemplate<K, V> re = (RedisTemplate<K, V>) redisTemplate;
        return re.opsForValue().get(key);
    }
     ...
     * @return
     */
    public <K, V> boolean set(K key, V value, int seconds) {
        try {
            RedisTemplate<K, V> re = (RedisTemplate<K, V>) redisTemplate;
            re.opsForValue().set(key, value, seconds);
            return true;
        } catch (Exception e) {
            e.printStackTrace();
            return true;
    ...
    public <T> List<T> getObject(String key, Class<T> clazz) {
        ...
        /*JSONArray jsonArray = (JSONArray) redisTemplate.opsForValue().get(key);
        return JSON.parseArray(JSON.toJSONString(jsonArray), clazz);*/
    }

    public List getList(List<String> keys) {
     ...
     * @return true成功 false失败
     */
    public boolean setString(String key, String value, long ms) {
        try {
    ...
    public long incr(String key, long delta, Integer time, TimeUnit timeUnit) {
        if (delta < 0) {
            throw new RuntimeException("递增因子必须大于0");
        }
